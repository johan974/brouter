...
import shortestdistance.util.BitCoderContext;
import shortestdistance.util.Crc32;
import shortestdistance.util.IByteArrayUnifier;
import shortestdistance.util.LruMap;
...
//    if ( histo.length == 500 ) continue;
  ...
  public void parseFile(File file, String readOnlyContext, Map<String, String> keyValues) {
    ...
    try {
      ...
      variableData = new float[variableNumbers.size()];
      if (minWriteIdx >= 0) System.arraycopy(readOnlyData, 0, variableData, 0, minWriteIdx);
  ...
  public void setVariableValue(String name, float value, boolean create) {
    ...
    if (num != null) {
      ...
      variableData = new float[variableNumbers.size()];
      System.arraycopy(readOnlyData, 0, variableData, 0, minWriteIdx);
      variableData[num] = value;
    }
  }
